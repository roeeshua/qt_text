...
//工具栏
    pushbotton=new QPushButton;
    pushbotton->setText("显示学号姓名");
    ui->mainToolBar->addWidget(pushbotton);
    spinFontSize = new QSpinBox;// 工具栏上的文字大小 SpinBox
    spinFontSize->setMinimum(5);
    spinFontSize->setMaximum(50);
    spinFontSize->setValue(ui->txtEdit->font().pointSize());//初始值
    spinFontSize->setMinimumWidth(100);//设置组件最小宽度
...
void QWMainWind::on_pushbutton_clicked(){
    QMessageBox::information(this, "About", "2022414120109\n黄展鸿");
}
...
{ //信号与槽的关联，当函数带有参数时，必须写明参数的类型
    ...
    connect(pushbotton,SIGNAL(clicked()),this,SLOT(on_pushbutton_clicked()));
}
